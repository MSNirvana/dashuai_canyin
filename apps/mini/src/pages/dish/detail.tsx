...
export default function DishDetailPage() {
  ...
  const isCombo = dish.kind === 'COMBO'
  const items = dish.comboItems ?? []
  /**
   * 套餐的「省多少」。
   * ★ 只在两个价格都拿到、且原价确实更高时才算：`originalPriceFen > priceFen` 是服务端的硬约束，
   *   但历史数据或前端算错都可能出现相等/更低 —— 那种情况下显示「省 ¥-8」比不显示更糟。
   */
  const savedFen = isCombo && dish.originalPriceFen && dish.priceFen !== null && dish.originalPriceFen > dish.priceFen
    ? dish.originalPriceFen - dish.priceFen
    : 0
  return <View className='dish-detail'><View className='dish-detail__hero'>{images[0] ? <Image src={images[0].url} mode='aspectFill' onClick={() => previewImages(0)} /> : <View className='dish-detail__placeholder'>{isCombo ? '暂无套餐封面' : '暂无封面'}</View>}</View><View className='dish-detail__body'>
    <View className='dish-detail__name-row'>
      <Text className='dish-detail__name'>{dish.name}</Text>
      {isCombo && <Text className='dish-detail__badge'>套餐</Text>}
    </View>
    {isCombo && (
      <View className='dish-detail__price-row'>
        <Text className='dish-detail__price'>¥{fenToYuan(dish.priceFen ?? 0)}</Text>
        {!!dish.originalPriceFen && <Text className='dish-detail__price-was'>¥{fenToYuan(dish.originalPriceFen)}</Text>}
        {savedFen > 0 && <Text className='dish-detail__price-save'>省 ¥{fenToYuan(savedFen)}</Text>}
      </View>
    )}
    {isCombo && (
      <View className='dish-detail__section'>
        <Text className='dish-detail__label'>套餐内容</Text>
        {items.length ? (
          <View className='dish-detail__combo'>
            {items.map((it) => (
              <View className='dish-detail__combo-row' key={it.id}>
                <Text className='dish-detail__combo-name'>{it.name}</Text>
                {it.quantity > 1 && <Text className='dish-detail__combo-qty'>×{it.quantity}</Text>}
              </View>
            ))}
          </View>
        ) : (
          <Text className='dish-detail__muted'>这个套餐还没有添加菜品</Text>
        )}
      </View>
    )}
    {dish.sellingPoints && <View className='dish-detail__section'><Text className='dish-detail__label'>卖点</Text><Text>{dish.sellingPoints}</Text></View>}{dish.intro && <View className='dish-detail__section'><Text className='dish-detail__label'>简介</Text><Text>{dish.intro}</Text></View>}<View className='dish-detail__section'><Text className='dish-detail__label'>建议出镜</Text><Text>{dish.sellingPoints ? (isCombo ? '先拍套餐全景，再逐样拍特写' : '先拍卖点，再拍一口下饭') : '补充卖点后，生成更贴合的分镜'}</Text></View><Text className='dish-detail__label'>全部图片</Text><View className='dish-detail__gallery'>{images.length ? images.map((m, i) => <Image key={m.cosKey} src={m.url} mode='aspectFill' onClick={() => previewImages(i)} />) : <Text className='dish-detail__muted'>暂无图片</Text>}</View><Text className='dish-detail__label'>全部视频</Text><View className='dish-detail__videos'>{videos.length ? videos.map((m) => <View className='dish-detail__video' key={m.cosKey}>{m.url ? <Video src={m.url} controls showCenterPlayBtn poster={m.coverUrl} /> : <Text>视频暂不可播放</Text>}</View>) : <Text className='dish-detail__muted'>暂无视频</Text>}</View></View><View className='dish-detail__footer'><View onClick={edit}>{isCombo ? '编辑套餐' : '编辑菜品'}</View></View></View>
}
