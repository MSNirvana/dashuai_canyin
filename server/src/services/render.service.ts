...
import { freeze, consume, BeanNotEnoughError, availableOf, type Db } from '../bean/bean.service.js'
...
export async function submitRender(
  ...
  // 幂等：同一 requestId 已提交过则直接返回原任务
  const existed = await prisma.renderTask.findFirst({ where: { requestId: reqId, merchantId } })
  if (existed) return { task: toView(existed), duplicated: true }
  ...
  const task = await prisma.$transaction(async (tx: Db) => {
    ...
    return tx.renderTask.create({
      data: {
        merchantId,
        creationId,
        status: grade === 'PREMIUM' ? 'MANUAL_PENDING' : 'QUEUED',
        grade,
  ...
  try {
    await prisma.$transaction(async (tx: Db) => {
      ...
      // 真实环境（FFMPEG_WORKER=true）：只做 freeze 预留，任务留给 worker 拉取。
      //   worker 成功 → consume 结算；失败 → unfreeze 全额释放（见 render/worker.ts）
      // PREMIUM：无论演示/真实环境都只 freeze 预留，任务停留在 MANUAL_PENDING，
      //   由管理后台剪辑工作台交付（deliver）结算，或超时 sweeper 退款
      if (simulateWorkerEnabled() || grade === 'PREMIUM') return

      // 演示环境（无 ffmpeg）：同步模拟成功，打通最小闭环
      await consume(tx, {
        merchantId,
        requestId: `rc:${task.id.toString()}`,
        amount,
        bizType: 'RENDER',
        bizId: task.id.toString(),
      ...
      await tx.renderTask.update({
        ...
        data: {
          ...
          progress: 100,
          beanCharged: amount,
          cacheHit: mode === 'RECOLOR',
          resultKey: clips[0]?.cosKey ?? null, // 演示：指向首个素材，真实环境为拼接产物 cosKey
          previewKey: clips[0]?.coverKey ?? null,
          durationMs: totalMs || null,
          finishAt: new Date(),
          costMs: Math.max(1, Math.round(totalMs / 1000) || 1),
        },
      })
    })
  } catch (e) {
    // 豆不足：freeze 阶段就被拦截（此时尚未 consume），标记 FAILED 即可，无需退款
    if (e instanceof BeanNotEnoughError) {
      await prisma.renderTask
        .update({
          where: { id: task.id },
          data: { status: 'FAILED', errorCode: 'BEAN_NOT_ENOUGH', errorMsg: 'AI豆不足' },
        })
        .catch(() => undefined)
      throw e
    }
  ...
  return { task: view, duplicated: false }
}
