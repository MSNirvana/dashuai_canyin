...
> = {
  ...
  /**
   * 封面选帧（**视觉**文本场景，2026-09-24 新增）。
   *
   * ★ 候选链**只有 GPT 一个**，这是刻意的：它每次要喂进去最多 6 张候选帧，
   *   而备用链上的 DeepSeek **是否接受图片输入没有验证过**（Claude 同理）。
   *   把没验证过的通道留在链上，失败时会**静默降级**到它 ——
   *   表现为「选帧结果莫名其妙」而不是报错，比直接失败难查得多。
   *   所以宁可 `fallbacks: []`（失败就回落到「选第 0 张」这个兜底），
   *   也不留一个没验证过的备用。
   *   ⚠ 要把 Claude / DeepSeek 加回来，先按本项目一贯的口径各打一轮**带图**请求再动
   *     （本项目反复吃过「凭直觉配候选链」的亏，见本文件 storyboard_generate 那段）。
   * ★ `maxOutputTokens: 2000` 而不是让 [3/6] 抬到默认的 4000：它只需要吐一句
   *   `{"index":2,"reason":"…"}`，输出预算给大了只会让推理模型的**思考**更奢侈、更慢。
   *   但也**不能**压到 1000 —— 思考吃光预算会返回空正文（BAD_RESPONSE，会按 maxRetries 重试）。
   * ★ `timeoutMs: 120_000`：带图请求明显慢于纯文本。
   *   本机实测（2026-09-24，6 张候选帧）：正常 **5.6 ~ 19.5s**；但同一天另一次真实调用
   *   **超过 90s**（同一时段出图也从 36s 涨到 85s ⇒ 中转站整体变慢）。
   *   取 120s ≈ 实测正常值的 6 倍，但**不能再抬**：前端总超时 ≈
   *   Σ(候选数 × 单场景超时 × (maxRetries+1))，当前 180(文本) + 120(本) + 90(封面) = 390s，
   *   刚好压在前端 420s 之内（apps/mini 的 PUBLISH_TIMEOUT_MS）—— 顶穿它就会变成
   *   「前端还在转圈、网关先掐」。
   *   ⚠ 390s **不含**「下载素材 + 抽帧」那一段（它不设场景超时、也不进上面这个 Σ），
   *     而它叠在 390s 之上 ⇒ 420 里留给它的只有 30s。**给抽帧加活也算改这个预算**
   *     （2026-09-24 起每段素材要抽两份：640 给模型看、1080 当底图）。
   *   ⚠ 还要记得：本场景超时会**熔断整个 tokenbox-gpt 通道 300s**（通道级熔断），
   *     把同通道的 publish_material / publish_cover 一起冷藏 ⇒ 超时越短越好，只是要够用。
   */
  publish_cover_pick: {
