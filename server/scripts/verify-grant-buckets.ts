/**
 * 赠积分分桶验证：注册赠积分必须独立于会员赠积分，**不随会员到期被清零**。
 *
 * 背景（第一批遗留问题，本轮修复）：
 *   bean_account 原先只有一个 grant_balance，注册赠积分（新用户一次性发 30）与会员赠积分
 *   （买会员送，到期清零）都往这里进，grant() 又把 bizType 硬编码成 'MEMBERSHIP'，
 *   账务上无法区分来源。会员到期时 expireGrant() 清空整个池子 → 把注册赠积分一起清掉。
 *
 * 为什么必须靠「分桶」而不是「改清零算法」：
 *   消耗是池化的（grant 桶整体扣减，没有批次概念），所以「账上还剩多少注册赠积分」
 *   无法从历史流水反推。只有独立成桶才能精确表达。
 *
 * 用例：
 *   ① 注册赠积分进注册桶，会员桶不动
 *   ② 会员赠积分进会员桶，注册桶不动
 *   ③ 消耗顺序：会员桶 → 注册桶 → 充值积分（先用会作废的）
 *   ④ ★ 会员到期清零：只清会员桶，注册桶原样保留
 *   ⑤ 幂等：同 bizId 重复发放只记一次
 *   ⑥ 可用额度含注册桶（否则用户看得见却用不了）
 *   ⑦ 对外 grantBalance 仍是「赠积分总量」
...
console.log(
  `商户 ${M} 基线：balance=${snapshot.balance} 会员赠积分=${snapshot.grantBalance} 注册赠积分=${snapshot.grantRegisterBalance} frozen=${snapshot.frozen}\n`,
)
...
console.log('=== ① / ② 两种来源分别进不同的桶 ===')
{
  ...
  await prisma.$transaction((tx) =>
    grant(tx, { merchantId: M, amount: 30n, source: 'REGISTER', bizId: `t1-${tag}`, remark: '注册赠积分' }),
  )
  let acc = await readAccount()
  check(acc.grantRegisterBalance === 30n, '① 注册赠积分 → 注册桶 = 30', `actual=${acc.grantRegisterBalance}`)
  ...
  check(acc.grantBalance === 98000n, '② 会员赠积分 → 会员桶 = 98000', `actual=${acc.grantBalance}`)
...
console.log('\n=== ③ 消耗顺序：会员桶 → 注册桶 → 充值积分 ===')
{
  ...
  check(acc.balance === 1000n, '充值积分还没动 = 1000', `actual=${acc.balance}`)

  const row1 = await prisma.beanLedger.findFirstOrThrow({
    where: { merchantId: M, requestId: req1, type: 'CONSUME' },
  })
  check(row1.grantAmount === 120n, '流水 grantAmount = 120（赠积分总量）', `actual=${row1.grantAmount}`)
  ...
  check(acc.balance === 970n, '差额 30 走充值积分，剩 970', `actual=${acc.balance}`)
  await cleanupLedger()
}

console.log('\n=== ④ ★ 核心用例：会员到期清零，注册赠积分必须保留 ===')
{
  ...
  check(row.grantRegisterAmount === 0n, 'EXPIRE 不含注册赠积分', `actual=${row.grantRegisterAmount}`)
  check(row.grantAfter === 30n, '流水 grantAfter = 剩余赠积分 30', `actual=${row.grantAfter}`)

  const bal = await getBalance(prisma, M)
  check(bal.available === 30n, '清零后可用额度 = 30（注册赠积分仍可用）', `actual=${bal.available}`)
  await cleanupLedger()
}
...
console.log('\n=== ⑤ 幂等：同 bizId 重复发放只记一次 ===')
{
  ...
  check(acc.grantRegisterBalance === 30n, '注册赠积分只发了一次 = 30', `actual=${acc.grantRegisterBalance}`)
...
console.log(
  `\n已恢复商户 ${M} 基线：balance=${restored.balance} 会员赠积分=${restored.grantBalance} 注册赠积分=${restored.grantRegisterBalance} frozen=${restored.frozen}`,
)
