...
卖点：{{sellingPoints}}
门店人设：{{persona}}
用户对怎么拍的要求（最高优先级，与下面冲突时以它为准）：{{userIdea}}
...
【用户对怎么拍的要求 · 本次最高优先级】{{userIdea}}

写作要求：
0. ★ 上面【用户对怎么拍的要求】优先级最高：与下面任何一条冲突时，一律以用户的要求为准；用户没有提到的方面，再按下面的要求发挥
...
输出要求：
0. ★ 上面【用户对怎么拍的要求】优先级最高：在**不违反下面第 1 条（分镜数量）与第 2 条（字段格式）**的前提下，一律按用户的要求安排画面、镜头与台词（例如用户点名要老板出镜、要拍现炒过程，就必须安排对应镜头）；用户没提到的方面再按下面的要求发挥
